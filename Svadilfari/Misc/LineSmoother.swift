...
class LineSmoother {
    ...
    static func getControlPoints(data: [CGPoint]) -> [BezierSegmentControlPoints] {
        ...
        if segments == 1 {
            ...
            let solution1 = Self.thomasAlgorithm(bd: bd, d: d, ad: ad, rhsArray: rhsArray, segments: segments, data: data)
            return solution1
        }
        return []
    }

    static private func thomasAlgorithm(bd: [CGFloat], d: [CGFloat], ad: [CGFloat], rhsArray: [CGPoint], segments: Int, data: [CGPoint]) -> [BezierSegmentControlPoints] {
        ...
        for i in (0..<segments) {
            if i == (segments - 1) {
                ...
                guard let controlPoint1 = p1 else {
                    continue
                }
        ...
        for i in (0..<segments) {
            guard let firstCP = firstControlPoints[i] else {
                continue
            }
            guard let secondCP = secondControlPoints[i] else {
                continue
            }
            let segmentControlPoint = (firstCP, secondCP)
