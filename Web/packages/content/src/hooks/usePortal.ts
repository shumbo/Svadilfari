 ...
 * @returns {HTMLElement}
 */
function createRootElement(tagName: TagName, id: string) {
  const rootContainer = document.createElement(tagName);
 ...
 * exists.
 * @example
 * const target = usePortal(tagName, id);
 * return createPortal(children, target);
 * @param {String} tagName tag to create
 * @param {String} id The id of the target container, e.g 'modal' or 'spotlight'
 * @returns {HTMLElement} The DOM node to use as the Portal target.
 */
export function usePortal(tagName: TagName, id: string): Element {
